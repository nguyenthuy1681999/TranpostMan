...
class driverPage extends Component {
  ...
  onFilter = (
    ...
    this.setState({
      filter: {
        ...
        filterCMT: filterCMT.toLowerCase(),
        filterMaSoBangLai: filterMaSoBangLai.toLowerCase(),
  ...
  render() {
    ...
    if (filter) {
      ...
      if (filter.filterCMT) {
        driver = driver.filter((item) => {
          return item.cmt.toLowerCase().indexOf(filter.filterCMT) !== -1;
        });
      }
      if (filter.filterMaSoBangLai) {
        driver = driver.filter((item) => {
          return (
            item.maSoBangLai.toLowerCase().indexOf(filter.filterMaSoBangLai) !==
            -1
          );
        });
      }
      if (filter.filterLoaiBangLai) {
        driver = driver.filter((item) => {
          return (
            item.loaiBangLai.toLowerCase().indexOf(filter.filterLoaiBangLai) !==
            -1
          );
        });
      }
